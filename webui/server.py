        ...
        async def write_now(bid: str):
            ...
            <div style="text-align:center;padding:60px 20px;">
              <div style="font-size:64px;margin-bottom:16px;">
                <div style="width:48px;height:48px;border:4px solid var(--neutral-200);border-top-color:var(--primary);border-radius:50%;animation:spin .8s linear infinite;margin:0 auto 20px;"></div>
              </div>
              <h2 style="border:none;font-size:20px;margin-bottom:8px;">✍️ 正在续写中...</h2>
              <p style="color:var(--neutral-400);margin-bottom:20px;">AI 正在生成下一章内容，请稍候...</p>
              <p style="color:var(--neutral-400);font-size:13px;" id="loadingStatus">⏳ 预计需要 15-60 秒</p>
              <div id="loadingSpinner" style="width:48px;height:48px;border:4px solid var(--neutral-200);border-top-color:var(--primary);border-radius:50%;animation:spin .8s linear infinite;margin:0 auto 20px;"></div>
            </div>
            <style>@keyframes spin{{to{{transform:rotate(360deg)}}}}</style>
            <script>
            (async function poll(){{
              var r=await fetch('/book/{bid}/write/check');
              var d=await r.json();
              if(d.done){{
                if(d.ok){{window.location.href='/book/{bid}';}}
                else{{document.getElementById('loadingStatus').textContent='💬 '+d.msg;document.getElementById('loadingSpinner').style.display='none';}}
              }}
...
document.querySelectorAll('form[data-loading]').forEach(function(f){f.addEventListener('submit',function(){
  ...
  if(btn){btn.disabled=true;btn.style.opacity=0.5;}
        ...
        {spinner}
        <nav class="nav"><span class="title">📖 AirNovel</span><a href="/">书籍概览</a><a href="/create">新建书籍</a></nav>
